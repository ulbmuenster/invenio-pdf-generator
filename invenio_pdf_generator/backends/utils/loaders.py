...
"""Template loaders for pdf-generator backend."""
...
from invenio_notifications.backends.utils import JinjaTemplateLoaderMixin
...
def unit_calculator(value: int) -> str:
    ...
    if value < 1000:
        ...
        return f"{floor(value / 1000)} KB"
    elif value < (1000 * 1000 * 1000):
        return f"{floor(value / (1000 * 1000))} MB"
    elif value < (1000 * 1000 * 1000 * 1000):
        return f"{floor(value / (1000 * 1000 * 1000))} GB"
    elif value < (1000 * 1000 * 1000 * 1000 * 1000):
        return f"{floor(value / (1000 * 1000 * 1000 * 1000))} TB"
    elif value < (1000 * 1000 * 1000 * 1000 * 1000 * 1000):
        return f"{floor(value / (1000 * 1000 * 1000 * 1000 * 1000))} PB"
    elif value < (1000 * 1000 * 1000 * 1000 * 1000 * 1000 * 1000):
        return f"{floor(value / (1000 * 1000 * 1000 * 1000 * 1000 * 1000))} EB"
    elif value < (1000 * 1000 * 1000 * 1000 * 1000 * 1000 * 1000 * 1000):
        return f"{floor(value / (1000 * 1000 * 1000 * 1000 * 1000 * 1000 * 1000))} ZB"
    return str(value)


class JinjaPdfTemplateLoaderMixin(JinjaTemplateLoaderMixin):
    """Used only in EmailWithPdfAttachmentNotificationBackend."""

    pdf_template_folder = "invenio_pdf_generator"
    ...
    def render_template(self, notification, recipient):
        ...
        current_app.jinja_env.filters["unit_calculator"] = unit_calculator
    ...
    def render_pdf_template(self, notification, recipient):
        """Render template for a notification."""
        locale = recipient.data.get("preferences", {}).get("locale")
        if not current_i18n.is_locale_available(locale):
            locale = get_locale()
        ...
        template = current_app.jinja_env.select_template(
            [
                # Default templates, e.g notifications/comment_edit.jinja
                f"{self.pdf_template_folder}/{notification.type}.{locale}.jinja",
                f"{self.pdf_template_folder}/{notification.type}.jinja",
            ]
        )
        ctx = template.new_context(
            {
                "notification": notification,
                "recipient": recipient,
                "image_path": f"{current_app.instance_path}/templates/semantic-ui/{self.pdf_template_folder}/images",
            },
        )
        ...
        css = None
        with open(
            f"{current_app.instance_path}/templates/semantic-ui/{self.pdf_template_folder}/css/style.css",
            "r",
        ) as f:
            string_css = f.read()
            css = CSS(string=string_css, font_config=font_config)
